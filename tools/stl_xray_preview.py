...
Requires (on top of the existing FluoroSim requirements.txt):
    pip install trimesh manifold3d --break-system-packages
    (manifold3d gives a guaranteed-watertight boolean difference — needed
    because a naive plane-slice can leave the cut face open on irregular
    cross-sections, which breaks gVXR's ray-casting; see clip_bottom().)
...
def parse_axis(spec: str) -> tuple[float, float, float]:
    ...
    Used for --ap-beam/--ap-up/--lat-beam/--lat-up overrides so orientation
    hypotheses (e.g. "AP beam is actually along Z, not Y") can be tested
    directly against a render instead of guessed at in config.py.
    """
    s = spec.strip().lower()
...
def clip_bottom(
    ...
    remainder as a new STL at `out_path`.

    Implementation: boolean DIFFERENCE against a large cutter box, via the
    `manifold3d` engine, rather than trimesh's slice_mesh_plane(cap=True).
    slice_mesh_plane's cap step triangulates the cut cross-section as a flat
    2-D polygon (via shapely + an earcut/triangle engine) and stitches it
    on — for a simple convex cross-section (like a plain box) this works,
    but for a real vertebra's irregular, possibly multi-loop cross-section
    at the cut plane, that triangulation step can fail to produce a closed
    boundary, leaving the mesh open. That's what happened on your STL last
    run (the "not watertight, fill_holes() also failed" warning was real —
    it wasn't cosmetic, it's almost certainly what caused the blown-out
    white renders, since gVXR's ray-casting needs a closed manifold surface
    to compute path length through the material; an open mesh with a hole
    can make the ray-caster treat everything past the hole as "still inside
    the material," which reads as near-total absorption -> flat white
    after the log/normalize/invert pipeline).

    `manifold3d`'s boolean ops guarantee manifold (closed, watertight)
    output for manifold input, regardless of cross-section complexity, so
    this replaces the plane-slice-and-cap approach entirely.
    ...
    or if the input mesh isn't watertight to begin with (boolean ops need a
    valid solid on both sides — check the raw STL if this fires).
    """
    ...
    if not mesh.is_watertight:
        raise RuntimeError(
            f"{stl_path} is not watertight BEFORE clipping — boolean "
            "difference needs a valid closed solid on both operands. Fix/"
            "re-export the source STL (e.g. re-export from Fusion with "
            "'repair' or run it through Meshmixer/Blender's remesh) before "
            "clipping. Use --no-clip to confirm this is actually the "
            "blocker (the render will still be geometrically wrong with "
            "the plate included, but tells you clipping isn't the cause)."
        )
    ...
    cut_at = bounds[0][axis_idx] + clip_mm

    # Cutter: a box far larger than the mesh in every direction, positioned
    # so its face along `up_axis` sits exactly at the cut height, covering
    # everything below it. Difference removes whatever overlaps the cutter.
    pad = float(np.max(bounds[1] - bounds[0])) * 4.0 + 100.0
    cutter = trimesh.creation.box(extents=[pad, pad, pad])
    shift = np.zeros(3)
    for i in range(3):
        if i == axis_idx:
            shift[i] = cut_at - pad / 2.0
        else:
            shift[i] = (bounds[0][i] + bounds[1][i]) / 2.0  # centred on mesh
    cutter.apply_translation(shift)

    clipped = trimesh.boolean.difference([mesh, cutter], engine="manifold")
    ...
    if not clipped.is_watertight:
        # Should not happen with the manifold engine, but don't silently
        # hand gVXR a broken mesh if it somehow does.
        raise RuntimeError(
            "Boolean difference produced a non-watertight result — this "
            "would reproduce the white-render bug. Inspect the source STL "
            "for self-intersections or degenerate faces (trimesh.repair)."
        )
...
def build_preview_cfg(
    ...
    ap_beam/ap_up/lat_beam/lat_up let you override config.py's beam/up
    vectors for this render only — use this to test an orientation
    hypothesis (e.g. "AP beam should be along Z, not Y") before committing
    it to config.py, rather than editing config.py and re-running blind.
    """
